...
namespace Logger {
	...
	using Log::Severity::critical;
	using Log::Severity::prohibit;
	...
	public:
		...
		struct Sink {
			SinkID id{};
			Severity min_skse{ prohibit };
			Severity min_papyrus{ prohibit };
			Callback_t* callback;
		};
		...
		SinkID AddSink(Callback_t* callback, const Severity min_skse, const Severity min_papyrus) noexcept {
			SinkID id{ next++ };
			exclusive_locker locker{ lock };
			try {
				sinks.emplace_back(id, min_skse, min_papyrus, callback);
				return id;
			}
			catch (...) {}
			return SinkID{ 0 };
		...
		void DistributeSKSEMessage(in_msg_type msg, const Severity severity) const noexcept {
			if (severity < prohibit) {
				shared_locker locker{ lock };
				for (const auto& sink : sinks) {
					if (sink.min_skse <= severity) {
						sink.callback(msg);
					}
				}
			}
	...
	public:
		...
		bool Init(const path& init_filepath) noexcept {
			exclusive_locker locker{ lock };
			if (ofs.is_open()) {
				return false;
			}
	...
	FileHandler& file_handler{ FileHandler::GetSingleton() };
	...
	bool Log::Init(const path& init_filepath) noexcept { return file_handler.Init(init_filepath); }


	Log::SinkToken Log::RegisterSink(Callback_t* callback, Severity min_skse, Severity min_papyrus) noexcept { return SinkToken{ sink_holder.AddSink(callback, min_skse, min_papyrus) }; }
	...
	void Log::NativeMessage(in_msg_type newmsg, const Severity severity) noexcept {
		try {
			static auto prefix = [](const Severity severity = prohibit) {
